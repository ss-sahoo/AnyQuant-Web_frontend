...
export function BelowSettingsModal({ onClose, currentInp1, initialSettings, onSave, onNext }: BelowSettingsModalProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedVolumeSettings) {
        ...
        console.log('🔍 Loaded saved Volume settings in below modal:', parsedSettings);
        if (parsedSettings.maLength) {
          setVolumeMaLength(Number(parsedSettings.maLength) || 20);
        }
      }
    } catch (error) {
      console.log('Error reading saved Volume settings in below modal:', error);
    }
  }, []);
    ...
    <>
      <DraggableModal onClose={onClose} className="sm:max-w-[425px] w-full max-h-[90vh] bg-white p-0 border border-gray-200 shadow-lg rounded-lg overflow-hidden flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <h2 className="text-lg font-medium text-black">Below Settings</h2>
