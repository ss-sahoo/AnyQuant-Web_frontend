...
export default function StrategyTestingPage() {
  ...
  // Function to update strategy settings via API
  const updateStrategySettings = async (cashValue: number, marginValue: number) => {
    try {
      if (!strategy_id) {
  ...
  const handleAccountDepositChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(numericValue)) {
      // Get current margin value from leverage
      const marginValue = getLeverageMargin(leverage)
      updateStrategySettings(numericValue, marginValue)
    }
  }
  ...
  const handleLeverageChange = (newLeverage: string) => {
    ...
    // Extract numeric value for API call
    const numericValue = Number.parseFloat(accountDeposit.replace(/,/g, ""))
    if (!isNaN(numericValue)) {
      const marginValue = getLeverageMargin(newLeverage)
      updateStrategySettings(numericValue, marginValue)
    }
  }
